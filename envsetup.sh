...
function check_product()
{
	...
	export BUILD_NUMBER=$( (date +%s%N ; echo $LEGION_BUILD; hostname) | openssl sha1 | sed -e 's/.*=//g; s/ //g' | cut -c1-10 )
    else
        LEGION_BUILD=
    fi
    export LEGION_BUILD
...
function lunch()
{
    ...
    if [ $? -ne 0 ]
    then
        # if we can't find a product, try to grab it off the legionOS-Devices GitHub
        T=$(gettop)
        cd $T > /dev/null
